...
-- http_client, quackapi (community). What changed from the previous version: the seven-arm
...
SELECT * FROM (VALUES
  ('.md',       'read_markdown',     $a$content_as_varchar := true, filename := true, include_filepath := true, normalize_content := true, extract_extensions := '', extract_metadata := true, maximum_file_size := 2000000, flavor := 'gfm', include_stats := true$a$),
  ('.markdown', 'read_markdown',     $a$content_as_varchar := true, filename := true, include_filepath := true, normalize_content := true, extract_extensions := '', extract_metadata := true, maximum_file_size := 2000000, flavor := 'gfm', include_stats := true$a$),
  ('.yaml',     'read_yaml_objects', $a$strip_document_suffixes := true, sample_size := 20480, ignore_errors := true, maximum_file_size := 2000000, maximum_sample_files := -1, maximum_object_size := 16777216, multi_document := false, auto_detect := true$a$),
  ('.yml',      'read_yaml_objects', $a$strip_document_suffixes := true, sample_size := 20480, ignore_errors := true, maximum_file_size := 2000000, maximum_sample_files := -1, maximum_object_size := 16777216, multi_document := false, auto_detect := true$a$),
  ('.html',     'read_html_objects', $a$filename := true, maximum_file_size := 2000000, ignore_errors := true$a$),
  ('.htm',      'read_html_objects', $a$filename := true, maximum_file_size := 2000000, ignore_errors := true$a$),
  ('.jsonl',    'read_json_objects', $a$filename := true, ignore_errors := true, format := 'newline_delimited'$a$),
  ('.json',     'read_json_objects', $a$filename := true, ignore_errors := true, format := 'array'$a$),
  ('.csv',      'read_csv',          $a$auto_detect := true$a$),
  ('.sql',      'read_text',         ''),
  ('.py',       'read_text',         ''),
  ('.sh',       'read_text',         ''),
  ('.txt',      'read_text',         '')
...
-- the path as a literal: chr(39) is the quote
stmts AS (
  SELECT f.*, r.reader,
         format('SELECT {}{}{} AS path, * FROM {}({}{}{}{})',
                chr(39), f.path, chr(39), r.reader, chr(39), f.path, chr(39),
...
responses AS (
  SELECT (u.e).path AS path, (u.e).reader AS reader,
         ((u.e).r).status AS status, (((u.e).r).body ->> '$') AS body, u.idx
  FROM fired CROSS JOIN UNNEST(responses) WITH ORDINALITY AS u(e, idx)
),
-- the route answers a JSON array of typed row objects; keep each row whole
reader_rows AS (
  SELECT path, reader, status, idx, row AS reader_row
  FROM responses, unnest(body::JSON[]) AS s(row)
  WHERE status = 200
),
...
line_stmts AS (
  SELECT DISTINCT path,
         format('SELECT {}{}{} AS path, line_number, content, byte_offset FROM read_lines({}{}{})',
                chr(39), path, chr(39), chr(39), path, chr(39)) AS q
  FROM reader_rows
),
line_fired AS (
...
line_rows AS (
  SELECT (u.e).path AS path, ((u.e).r).status AS status, row.line_number, row.content, row.byte_offset
  FROM line_fired CROSS JOIN UNNEST(responses) WITH ORDINALITY AS u(e, idx),
       unnest(from_json((((u.e).r).body ->> '$'),
                        '[{"line_number":"BIGINT","content":"VARCHAR","byte_offset":"BIGINT"}]')) AS s(row)
