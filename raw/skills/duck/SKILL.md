...
`/duckdb-skills:self-dispatch`: (1) **in-process httpserver** — `httpserve_start('127.0.0.1',
port, '')` in the same `:memory:` process, `array_agg(http_post_form(url, MAP{}, MAP{'q': q}))`,
`UNNEST WITH ORDINALITY`, NDJSON back, `httpserve_stop()`; (2) **two constant shellfs pipes** —
